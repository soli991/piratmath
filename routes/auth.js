...
function buildTopicsMap(userId) {
  const rows = db.prepare(
    'SELECT topic, done, points, week_done_easy, week_start_easy, week_done_med, week_start_med FROM topic_progress WHERE user_id = ?'
  ).all(userId);
  ...
  for (const row of rows) {
    topics[row.topic] = {
      ...
      points: row.points,
      dukats: dukatMap[row.topic] || 0,
      week_done_easy:  row.week_done_easy  || 0,
      week_start_easy: row.week_start_easy || '',
      week_done_med:   row.week_done_med   || 0,
      week_start_med:  row.week_start_med  || '',
    };
  }
  return topics;
}
